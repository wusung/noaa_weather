...
<label><input type="radio" name="sample_rate" value="r" checked />Raw</label>
<label><input type="radio" name="sample_rate" value="d" />1 Day</label>
...
var w = this, ws = WeatherSystem;(function(Backbone) {
	...
	(function(_Model, _Collection, _View) {
		...
		var StationCollection = Collection.extend({
			...
			*/
			comparator: function(a, b) {
				var stateA = a.get('state');
				var stateB = b.get('state');
				if (stateA == stateB) {
		...
		$(function() {
			...
			$(document).on('click', 'button', function(ev) {
				...
				var t = new buds.Template('<%=webapp%>/weather_station', _helper);
				var params = buds.Forms.serialize($('form')[0]);
				params['fields'] = _helper.join(params['col']);
				if (!params['station']) {
				...
				url = params.appendTo(url, ['station', 'begin_time', 'end_time', 'fields', 'sample_rate']);
				location.href = url;
				return false;
			});
		});
