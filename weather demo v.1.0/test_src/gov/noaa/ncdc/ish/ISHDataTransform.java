 ...
 * Mainline logic.
 */
public class ISHDataTransform {
	
	public static void main(String[] args) {
		File dir = new File("\\\\192.168.3.33/bu/weather/ready/");
		File[] yeardir = dir.listFiles();
		int cntDir = 0;
		for (int i = 0, len = yeardir.length; i < len; ++i) {
			if (!yeardir[i].exists() || !yeardir[i].isDirectory()) continue;
			processYear(yeardir[i]);
			++cntDir;
		}
		System.out.println(cntDir+" years DONE");
//		processYear(new File(dir, "2014"));
	}
	
//	public static void main(String[] args) {
//		File dir = new File("\\\\192.168.3.33/bu/weather/2007/raw/");
//		File outdir = new File("\\\\192.168.3.33/bu/weather/2007/csv/");
//		outdir.mkdirs();
//		long ts = System.currentTimeMillis();
//		File[] files = new File[] {
//			new File(dir, "070930-99999-2007.gz"),
//			new File(dir, "684880-99999-2007.gz"),
//			new File(dir, "722285-03896-2007.gz")
//		};
...
//			}
//			File src = files[i];
//			File tgt = new File(outdir, src.getName());
//			if (tgt.exists() && tgt.length() > 0)
//				continue;
//			// transfer(src, tgt);
//			Thread t = new Thread(new Transfer(src, tgt));
//			t.start();
//		}
//		while (counter.get() > 0) {
//			try {
...
//	}
	...
	public static void processYear(File dir) {
		File yetdir = new File(dir, "yet/");
		if (!yetdir.exists()) {
			yetdir = dir;
		}
		File rawdir = new File(dir, "raw/");
		rawdir.mkdirs();
		File outdir = new File(dir, "csv/");
		outdir.mkdirs();
		System.out.println("startup "+dir.getAbsolutePath());
		long ts = System.currentTimeMillis();
		File[] files = yetdir.listFiles();
		for (int i = 0, len = files.length; i < len; ++i) {
			File src = files[i];
			...
			File tgt = new File(outdir, src.getName());
			if (tgt.exists() && tgt.length() > 0) continue;
			File ahv = new File(rawdir, src.getName());
			// transfer(src, tgt);
			Thread t = new Thread(new Transfer(src, tgt, ahv));
			t.start();
		}
		while (counter.get() > 0) {
			try {
		...
		System.out.println(files.length+" files over "+(System.currentTimeMillis()-ts)+"ms");
	}
	...
	static class Transfer implements Runnable {
		...
		public Transfer(File src, File tgt, File ahv) {
			this.src = src;
			this.tgt = tgt;
			this.ahv = ahv;
