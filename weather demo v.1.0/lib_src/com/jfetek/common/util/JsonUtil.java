...
//	}
	...
	public static JSONObject getBasicJson(ErrorCode ec) {
		...
		JSONObject json = new JSONObject();
		try {
			json.put("status", ec.positive()? "ok" : "error")
//				.put("error", ec.toJson())
				.put("timestamp", System.currentTimeMillis());
			if (ec.negative()) {
				json.put("message", ec.message);
			}
		} catch (JSONException e) {}
		return json;
	}
	public static Object getBasicJson(Result<?> result) {
		if (null == result) return null;
		if (result.positive()) {
			return makeJsonize(result.data);
		}
		else {
			JSONObject json = new JSONObject();
			try {
			json.put("status", result.positive()? "ok" : "error")
//				.put("error", result.toJson())
	...
	// optional field: forward
	public static JSONObject addForwardField(JSONObject json, CharSequence forward_to, Params forward_params) {
		try {
			// optional field: forward
			if (forward_to != null) {
				JSONObject forward = new JSONObject().put("to", forward_to);
				if (forward_params != null) {
					forward.put("with", forward_params.toQueryString());
				}
	...
	public static JSONStringer addForwardField(JSONStringer json, CharSequence forward_to, Params forward_params) {
		try {
			// optional field: forward
			if (forward_to != null) {
				JSONObject forward = new JSONObject().put("to", forward_to);
				...
				json.key("forward").value(forward);
			}
		} catch (JSONException e) {}
		return json;
	}
