...
//		return !this.from.before(date) && !this.to.after(date);
	...
	public MonthOfYear[] getMonthOfYearArray() {
		TreeSet<MonthOfYear> moys = new TreeSet<MonthOfYear>();
		for (Date d = this.first; !d.after(this.last); d = d.next()) {
			moys.add( d.getMonthOfYear() );
		}
		return moys.toArray(new MonthOfYear[0]);
	}
	
//	public DateRange[] splitRangeMonthly() {
//		
//	}
